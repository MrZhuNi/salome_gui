...
void OCCViewer_ViewWindow::setProjectionType( int mode )
{
  ...
  if ( !aView3d.IsNull() ) {
    ...
    if (mode == Stereo) {
      ...
      setQuadBufferSupport( aResMgr->booleanValue( "OCCViewer", "enable_quad_buffer_support", true ) );
    }
    aView3d->Redraw();
    onViewFitAll();
  }
  ...
  if ( mode == Stereo ) {
    if ( anOrthographicAction->isEnabled() )
      anOrthographicAction->setDisabled( true );
    else
      anOrthographicAction->setEnabled( true );
    if ( aPerspectiveAction->isEnabled() ) {
      aPerspectiveAction->setChecked( true );
      aPerspectiveAction->setDisabled( true );
      if ( isQuadBufferSupport() ) {
        if ( !isOpenGlStereoSupport() && stereoType() == QuadBuffer )
