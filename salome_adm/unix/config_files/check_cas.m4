...
case $host_os in
  ...
  if test -n $GUI_ROOT_DIR; then
      if test -d $GUI_ROOT_DIR/include/salome; then
          CAS_CPPFLAGS="$CAS_CPPFLAGS -I$GUI_ROOT_DIR/include/salome"
          CPPFLAGS="$CPPFLAGS -I$GUI_ROOT_DIR/include/salome"
      fi
  fi
